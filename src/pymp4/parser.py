...
FreeBox = Struct("type" / Const("free".encode(STRING_ENCODING)), "data" / GreedyBytes)

SkipBox = Struct("type" / Const("skip".encode(STRING_ENCODING)), "data" / GreedyBytes)
...
EditListBox = Struct(
    "type" / Const("elst".encode(STRING_ENCODING)),
    "version" / Default(Int8ub, 0),
    "flags" / Const(0, Int24ub),
    "entry_count" / Default(Int32ub, 0),
    "data_entries"
    / Array(
        this.entry_count,
        Switch(
            ...
            {
                1: Struct(
                    "segment_duration" / Default(Int64ub, 0),
                    "media_time" / Default(Int64sb, 0),
                    "media_rate_integer" / Default(Int16sb, 0),
                    "media_rate_fraction" / Const(0, Int16sb),
                ),
                0: Struct(
                    "segment_duration" / Default(Int32ub, 0),
                    "media_time" / Default(Int32sb, 0),
                    "media_rate_integer" / Default(Int16sb, 0),
                    "media_rate_fraction" / Const(0, Int16sb),
                ),
            },
...
DataEntryUrnBox = Prefixed(
    ...
    Struct(
        ...
        "version" / Const(0, Int8ub),
        "flags" / BitStruct(Padding(23), "self_contained" / Rebuild(Flag, ~(this._.name & this._.location))),
...
SampleSizeBox2 = Struct(
    ...
    "entries"
    / Array(this.sample_count, Struct("entry_size" / LazyBound(lambda ctx: globals()["Int%dub" % ctx.field_size]))),
)
...
TrackSampleFlags = BitStruct(
    ...
    / Default(Enum(BitsInteger(2), UNKNOWN=0, NOTDISPOSABLE=1, DISPOSABLE=2, RESERVED=3, default=0), 0),
    "sample_has_redundancy"
    / Default(Enum(BitsInteger(2), UNKNOWN=0, REDUNDANT=1, NOTREDUNDANT=2, RESERVED=3, default=0), 0),
...
TrackRunBox = Struct(
    ...
    / Array(
        ...
        Struct(
            ...
            / If(
                this._.flags.sample_composition_time_offsets_present, IfThenElse(this._.version == 0, Int32ub, Int32sb)
            ),
        ),
    ),
)
...
MovieDataBox = Struct("type" / Const("mdat".encode(STRING_ENCODING)), "data" / GreedyBytes)
